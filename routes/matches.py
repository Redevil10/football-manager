...
def create_match_page(league_id: str = None, req: Request = None, sess=None):
    ...
    return render_create_match_page(
        user,
        sess,
        req,
        other_leagues,
...
async def route_update_match(match_id: int, req: Request, sess=None):
    ...
    if league_id_str:
        ...
        if club_ids:
            ...
            return RedirectResponse("/matches?error=No+clubs+assigned", status_code=303)
...
def edit_match_team_page(match_id: int, team_id: int, req: Request = None, sess=None):
...
def register_match_routes(rt):
    ...
    rt("/matches")(matches_page)
    rt("/create_match", methods=["GET"])(create_match_page)
    rt("/api/get_last_match/{league_id}")(api_get_last_match)
    rt("/create_match/{league_id}")(create_match_page_with_league)
    rt("/create_match", methods=["POST"])(route_create_match)
    ...
    rt("/edit_match/{match_id}")(edit_match_page)
    rt("/update_match/{match_id}", methods=["POST"])(route_update_match)
    rt("/delete_match/{match_id}", methods=["POST"])(route_delete_match)
    rt("/edit_match_team/{match_id}/{team_id}")(edit_match_team_page)
    rt("/update_match_team/{match_id}/{team_id}", methods=["POST"])(
        route_update_match_team
    )
    rt("/set_captain/{match_id}/{team_id}", methods=["POST"])(route_set_captain)
    ...
    rt("/import_match_players/{match_id}", methods=["GET"])(import_match_players_page)
    rt("/import_match_players/{match_id}", methods=["POST"])(route_import_match_players)
    rt("/confirm_import/{match_id}", methods=["POST"])(route_confirm_import)
    rt("/add_match_player_manual/{match_id}", methods=["GET"])(
        add_match_player_manual_page
    )
    rt("/add_match_player_manual/{match_id}", methods=["POST"])(
        route_add_match_player_manual
    )
    rt("/remove_match_signup_player/{match_id}/{match_player_id}", methods=["POST"])(
        route_remove_match_signup_player
    )
    rt("/remove_all_match_signup_players/{match_id}", methods=["POST"])(
        route_remove_all_match_signup_players
    )
    rt("/add_match_recordings/{match_id}", methods=["POST"])(route_add_match_recordings)
    rt("/delete_match_recording/{match_id}/{recording_id}", methods=["POST"])(
        route_delete_match_recording
    )
