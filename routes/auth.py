...
async def route_register(req: Request, sess=None):
    ...
    if not current_user_is_superuser:
        ...
        is_admin = any(club.get("role") == USER_ROLES["ADMIN"] for club in user_clubs)
    ...
    try:
        ...
        if user_id:
            ...
            logger.info(f"Assigned user {user_id} to club {club_id} with role {role}")
            ...
            if current_user_is_superuser and new_user_is_superuser and sess is not None:
...
def register_auth_routes(rt):
    ...
    rt("/change-password", methods=["POST"])(route_change_password)
    rt("/change-password")(change_password_page)
